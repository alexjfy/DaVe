...
class sensor_agent_monitor extends uvm_monitor;
  ...
  virtual sensor_interface                sensor_monitor_if;
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual sensor_interface)::get(this, "", "sensor_interface", sensor_monitor_if))
      `uvm_fatal("SENSOR_MONITOR", {"Virtual interface must be set for: ",get_full_name(),".sensor_monitor_if"})
  endfunction
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    //wait for reset
    @(negedge sensor_monitor_if.rst);
    @(posedge sensor_monitor_if.rst);
    @(sensor_monitor_if.clk);
    fork
      collect_and_send();
    join_none
  endtask
  ...
  task collect_and_send();
    forever begin
      @(negedge sensor_monitor_if.clk);
      if(sensor_monitor_if.valid == 1 && sensor_monitor_if.ready == 1) begin
        sensor_trans.sensor = sensor_monitor_if.brightness;
        @(negedge sensor_monitor_if.clk);
